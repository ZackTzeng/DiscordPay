...
async function transferHbar(fromAccountId, fromPriKey, toAccountId, amount) {
  ...
  const signTx = await sendHbarTx.sign(PrivateKey.fromString(fromPriKey));
  const txResponse = await signTx.execute(client);
}

async function transferToken(fromAccountId, fromPriKey, toAccountId, toPriKey, amount, tokenId) {
  console.log("fromAccountId ", fromAccountId)
  console.log("fromPriKey", fromPriKey);
  console.log("toAccountId", toAccountId)
  console.log("amount", amount)
  ...
  // force association
  try{
      await associateTokenWithAccount(toAccountId,toPriKey,tokenId);
  } catch(err) {
  ...
  var transaction = await new TransferTransaction()
    .addTokenTransfer(tokenId, fromAccountId, amount * -1)
    .addTokenTransfer(tokenId, toAccountId, amount)
    .freezeWith(client);

  //Sign with the sender account private key
  console.log("from private key ", fromPriKey)
  var signTx = await transaction.sign(PrivateKey.fromString(fromPriKey));
...
async function associateTokenWithAccount(accountId, priKey, tokenId) {
  ...
  const signTx = await transaction.sign(PrivateKey.fromString(priKey));
  ...
  console.log("association status ", transactionStatus)
